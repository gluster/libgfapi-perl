...
sub new
{
    ...
    $ffi->attach(glfs_new => ['string'] => 'pointer');
    $ffi->attach(glfs_set_volfile_server => ['pointer', 'string', 'string', 'int'] => 'int');
    $ffi->attach(glfs_init => ['pointer'], => 'int');
    $ffi->attach(glfs_fini => ['pointer'] => 'int');
